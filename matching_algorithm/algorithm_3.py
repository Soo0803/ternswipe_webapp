...
# Major-based bonus: stronger and generalized across majors, scaled by coverage
def major_bonus(row):
    sid = row["sid"]
    pid = row["pid"]
    ...
    # Base bonus increases with stronger alignment
    base = 0.20 if overlap >= 2 else 0.08
    # Scale with coverage so we don't over-reward zero/low coverage
    cov = float(row.get("coverage", 0.0))
    scale = 0.6 + 0.4*max(0.0, min(1.0, cov))  # 0.6..1.0 (less restrictive)
    return base * scale
...
# Coverage gating multiplier to avoid low-coverage outranking relevant candidates
def cov_gate(c):
    if c <= 0.0:
        return 0.2
    if c < 0.34:
...
test["gate"] = test["coverage"].apply(cov_gate)
...
def recompute(r):
    ...
    g = cov_gate(r["coverage"]) if "coverage" in r else 1.0
    mb = major_bonus(r)
