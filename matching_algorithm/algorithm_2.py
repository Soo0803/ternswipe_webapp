...
students = pd.DataFrame([
    # id, text (major/courses/skills), gpa_bucket, hrs, avail_weeks, reliability, shortcomings
    ("s1","Chemistry major; courses: Lab A-, Writing B; skills: lab, writing, data analysis",    "3.0-3.5", 6, 10, 0,0,0,0,0),
    ...
    ("s5","Humanities major; courses: DSP A, ML A-; skills: python, pytorch, signal processing", "0.0", 10, 16, 0,0,0,0,0),
...
projects = pd.DataFrame([
    # id, text (topic/reqs), req_hrs, dur_weeks, req_skills
...
# All possible student-project pairs
...
# -------------------------
# 2) Text similarity (TFIDF as stand-in for embeddings)
#    Swap to sentence-transformers in production.
# -------------------------
stu_vec = TfidfVectorizer(min_df=1).fit(students.st_text)
prj_vec = TfidfVectorizer(min_df=1).fit(projects.pr_text)
S = stu_vec.transform(students.st_text)   # [S x V1]
P = prj_vec.transform(projects.pr_text)   # [P x V2]
# To compare, embed both with same vocab in production; here we approximate:
# Use a joint vectorizer on concatenated corpus for demo
joint = TfidfVectorizer(min_df=1).fit(pd.concat([students.st_text, projects.pr_text]))
S2 = joint.transform(students.st_text)
P2 = joint.transform(projects.pr_text)
cos = cosine_similarity(S2, P2)  # [S x P]
...
for _,r in df.iterrows():
    ...
    rows.append({
        ...
        # shortcomings penalties (1 if present)
...
# Simple Fit proxy (blend of interpretable signals)
X["fit"] = 0.55*X["cosine"] + 0.30*X["coverage"] + 0.15*X["avail"]
# Penalties (tunable)
pen = 0.10*X["lack_commit"] + 0.07*X["poor_comms"] + 0.05*X["weak_writing"] + 0.05*X["weak_numeracy"]

# -------------------------
# 4) Train a ranking/acceptance model (offline)
# -------------------------
...
if HAVE_LGBM:
    ...
    def groups(df_):
        return df_.groupby("sid").size().tolist()
    ranker = LGBMRanker(n_estimators=200, learning_rate=0.1, max_depth=-1, random_state=42)
...
# Performance proxy (could be a separate model; here we reuse fit+gpa)
...
# -------------------------
# 5) Offline metrics: AUC, Precision@k, NDCG@k
# -------------------------
...
def ndcg_at_k(df, k=3):
    def dcg(y):
        return np.sum([(2**rel - 1)/np.log2(i+2) for i,rel in enumerate(y)])
    out=[]
    for sid, grp in df.sort_values("score", ascending=False).groupby("sid"):
        y_true = grp["y"].values[:k]
        ideal = np.sort(grp["y"].values)[::-1][:k]
        out.append(dcg(y_true)/max(dcg(ideal),1e-9))
    return float(np.mean(out)) if out else 0.0

p3 = precision_at_k(test, k=3)
n3 = ndcg_at_k(test, k=3)

print(f"AUC={auc:.3f}  P@3={p3:.3f}  NDCG@3={n3:.3f}")

# -------------------------
# 6) Sanity checks (unit-test-style assertions)
# -------------------------
# (a) Coverage matters: pairs with higher coverage should score higher on average
chk = test.copy()
by_cov = chk.assign(cbin=(chk.coverage>=0.5).astype(int)).groupby("cbin")["score"].mean().to_dict()
assert by_cov.get(1,0) >= by_cov.get(0,0), "Coverage not reflected in score"

# (b) Shortcomings penalize: if same student/project but flipped shortcomings, higher penalty → lower score
# (Synthetic check by perturbation)
...
assert recompute(row_p) <= base + 1e-9, "Penalty not decreasing score"
